...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    outline_point = generate_points_on_circle(circle, number_of_slices)

    for k in range(len(outline_point) + 1):
        line = rg.Line(circle.center, outline_point[k-1])
        line.color = color
...
def run_test_polygon():
    ...
    title = ('POLYGON tests 3:  green circle with square')
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    outline_point = generate_points_on_circle(circle, number_of_segments)

    for k in range(len(outline_point) + 1):
        line = rg.Line(outline_point[k - 2], outline_point[k - 1])
        line.color = color
        line.thickness = thickness
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    outline_point = generate_points_on_circle(circle, number_of_lines)

    for k in range(len(outline_point) + 1):
        line = rg.Line(outline_point[k - hops_to_next_point - 1], outline_point[k - 1])
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
